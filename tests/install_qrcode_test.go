...
var _ = Describe("c3os qr code install", Label("qrcode-install"), func() {
	...
	Context("install", func() {
		It("to disk with custom config", func() {
			...
			out, err = utils.SH(fmt.Sprintf("EDGEVPNTOKEN=%s ./edgevpn fs --name screenshot --path %s &", os.Getenv("EDGEVPNTOKEN"), f.Name()))
			fmt.Println(out)
			Expect(err).ToNot(HaveOccurred(), out)

			// Wait for install to unblock
			Eventually(func() string {
				v, _ = machine.SSHCommand("cat /proc/cmdline")
				return v
